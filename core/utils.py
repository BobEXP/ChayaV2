 ...
 | (__| ' \/ _` | || / _` \ V / / / 
  \___|_||_\__,_|\_, \__,_|\_/ /___|
	...
	try:
		...
		print(f" {c_bold}{c_yellow}[ v2 ] {c_red} [ 2022 ]{c_clean}")
	print(f" {c_blue}{c_bold} [ https://github.com/BobEXP ]{c_clean}")
...
async def download_file(url: str, download_with: str, dest=None) -> None:
	...
	filepath = f"{str(Path.cwd()).replace('core/utils.py', '')}downloads/{filename}"
